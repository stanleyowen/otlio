...
export default function App() {
  ...
  useEffect(() => {
    async function getData() {
      await axios
        .get(`${server}/account/user`, { withCredentials: true })
        .then((res) => {
          setUserData({
            status: res.status,
            isLoading: false,
        ...
        .catch((err) => {
          ...
          if (
            err.response.status === 302 &&
            err.response.data.type.verifyAccount &&
            window.location.pathname !== "/get-started" &&
            window.location.pathname !== "/logout" &&
            window.location.pathname !== "/support" &&
            window.location.pathname.split("/")[1] !== "verify"
          )
  ...
  useEffect(() => {
    ...
    async function ping(a) {
      await axios
        ...
        .catch((err) => {
          if (err.response && err.response.status >= 500 && server_list[a + 1])
            ping(a + 1);
  ...
  return (
    <Router>
      ...
      <Switch>
        ...
        <Route
          path="/get-started"
          component={() => <Register userData={userData} />}
        />
        <Route path="/login" component={() => <Login userData={userData} />} />
        <Route
          path="/logout"
          component={() => <Logout userData={userData} />}
        />
        <Route
          path="/edit/:id"
          component={() => <EditTodo userData={userData} />}
        />
        <Route
          path="/oauth"
          component={() => <ReqOAuth userData={userData} />}
        />
        <Route
          path="/auth/:service/:email"
          component={() => <OAuth userData={userData} />}
        />
        <Route
          path="/account"
        ...
        <Route
          path="/support"
          component={() => <Support userData={userData} />}
        />
        <Route
          path="/reset-password"
          exact
          component={() => <ReqResetPassword userData={userData} />}
        />
        <Route
          path="/reset-password/:id/:token"
          component={() => <ResetPassword userData={userData} />}
        />
        <Route
          path="/verify/:id/:token"
          component={() => <VerifyAccount userData={userData} />}
        />
        <Route path="/privacy-policy" component={PrivacyPolicy} />
        <Route path="/terms-and-conditions" component={TermsAndConditions} />
        <Route
          path="*"
          component={() => <PageNotFound userData={userData} />}
        />
      </Switch>
    </Router>
  );
