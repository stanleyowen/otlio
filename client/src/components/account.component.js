  ...
  const BackupCodes = () => {
    ...
    for (let x = 0; x < codes.length; x++) {
      if (x % 2 === 0 && x !== 0) {
        ...
        column.innerHTML = codes[x];
        row.innerHTML = column.outerHTML;
...
(stanleyowen06@gmail.com)`;
  ...
  const changePassword = (e) => {
    ...
    async function submitData() {
      ...
      handleChange("disabled", true);
      btn.innerText = "Updating...";
      btn.setAttribute("disabled", "true");
      btn.classList.add("disabled");
      await axios
        .put(
          `${SERVER_URL}/account/user`,
          { ...password, ...data },
    ...
    if (!authenticated)
      ...
      password.newPassword.length > 60 ||
      password.confirmPassword.length > 60
    ) {
      setNotification(
        NOTIFICATION_TYPES.DANGER,
      ...
      document
        .getElementById(
          password.newPassword.length < 6 || password.newPassword.length > 60
            ? "new-password"
            : "confirm-password"
        )
        .focus();
      ...
      if (!data.isBackupCode && !data.tokenId)
        setNotification(
          NOTIFICATION_TYPES.DANGER,
          "Please Click on Send Verification Code to Proceed !"
        );
      else if (!data.token) {
        setNotification(NOTIFICATION_TYPES.DANGER, "Authentication Required !");
        openModal("otp-bg", "otp-modal");
        document.getElementById("otp-token-1").focus();
      } else if (data.token && !isNum.test(data.token)) {
        setNotification(
          NOTIFICATION_TYPES.DANGER,
          "Please Provide a Valid OTP Code Format !"
        );
        document.getElementById("otp-token-1").focus();
      } else submitData();
    } else submitData();
  ...
  const sendOTP = (e) => {
    ...
    async function sendToken() {
      ...
      btn.classList.add("disabled");
      await axios
        .get(`${SERVER_URL}/account/otp`, { withCredentials: true })
        .then((res) => {
          setNotification(NOTIFICATION_TYPES.SUCCESS, res.data.message);
      ...
      passwordBtn.innerText = "Send Verification Code";
  ...
  const VerifyOTP = (e) => {
    ...
    async function submitData() {
      ...
      handleChange("disabled", true);
      await axios
        .put(`${SERVER_URL}/account/otp`, data, {
    ...
    else if (!data.isBackupCode && !data.tokenId)
      setNotification(
        NOTIFICATION_TYPES.DANGER,
        "Sorry, we are not able to process your request. Please try again later."
      );
  ...
  const RegenerateToken = (e) => {
    ...
    async function generateToken() {
      ...
      await axios
        .post(
          `${SERVER_URL}/account/backup-code`,
          { regenerate: true },
          { headers: { "XSRF-TOKEN": getCSRFToken() }, withCredentials: true }
        )
        .then((res) => {
    ...
    if (!security["2FA"])
      setNotification(
        ...
        "Backup Codes are only eligle for Two Factor Authentication (2FA) Users"
      );
    else generateToken();
  };
    ...
    <div>
      ...
      <div className="main">
        <div className="account-container">
          <div className="flex35 center-object p-5p">
            ...
            <img
              className={properties.mfa ? "" : "none"}
              src={Images["2FA"]}
              alt="Introducing 2FA"
              onLoad={() => handleChange("mfa", true)}
              onError={(e) => {
                e.target.src = Images.local2FA;
              }}
            />
            <h2 className="isCentered mt-20 mb-20 monospace">
              Introducing 2FA
            </h2>
            <p className="isCentered monospace">
              Two-Factor Authentication is a security control that requires
              users to verify their identities by providing multiple pieces of
              evidence before gaining access to a device or application.
            </p>
          </div>
          <div className="no-padding mb-40 flex65">
            <div className="form__contact no-margin">
              <div className="get_in_touch">
                <h1 className="monospace">Account</h1>
              </div>
              <div className="m-10">
              ...
              <div className="oauth-container">
                <div className="m-10">
                  <button
                    className="oauth-box primary mt-20"
                    onClick={() =>
                      openModal("password-bg", "password-modal", "old-password")
                    }
                  >
                    <FontAwesomeIcon icon={faKeycdn} size="2x" />{" "}
                    <p>Update Password</p>
                  </button>
                </div>
                <div className="m-10">
              ...
              <div className="get_in_touch mt-40">
                <h1 className="monospace">Security</h1>
              </div>
              <div className="form">
                <div className="m-10">
                  <FormControlLabel
                    control={
                      <Switch
                        checked={!isLoading ? security["2FA"] : false}
                        onClick={() =>
                          !isLoading ? openModal("mfa-bg", "mfa-modal") : null
                        }
                        color="primary"
                      />
                    }
                ...
                <div className="oauth-container">
                  <div className="m-10">
                    <button
                      className="oauth-box primary mt-20"
                      onClick={() =>
                        openModal("backup-code-bg", "backup-code-modal")
                      }
                    >
              ...
              <div className="get_in_touch mt-40">
                <h1 className="monospace">Third Party</h1>
              </div>
              <div className="contact__container mt-10">
                <p className="pr-10">
                  <button
                    className="oauth-box google mt-10 mb-10"
                    onClick={() =>
                  ...
                  >
                    ...
                    {!isLoading && thirdParty && thirdParty.google ? (
                    ...
                    <p>
                      {thirdParty ? (
                        thirdParty.google ? (
                          <span>
                            <span id="connect">Connected</span>
                ...
                <p className="pl-10">
                  <button
                    className="oauth-box github mt-10"
                    onClick={() =>
                      authenticated && SERVER_URL
                  ...
                  >
                    ...
                    {!isLoading && thirdParty && thirdParty.github ? (
                      <FontAwesomeIcon icon={faCheck} size="2x" />
                    ) : null}{" "}
                    <p>
                      {thirdParty ? (
              ...
              <p className="isCentered mt-20 mb-20">
                <a
                  className="link"
              ...
              <p className="isCentered mt-20 mb-20 bold raleway">
                &copy; {new Date().getFullYear()} Otlio - All Rights Reserved.
              </p>
            </div>
          </div>
        </div>
      ...
      <div id="password-bg" className="modal hiddenModal">
        <div id="password-modal" className="modal__container hiddenModal">
          <IconButton
            onClick={() => closeModal("password-bg", "password-modal")}
          ...
          <div className="modal__body">
            <form onSubmit={changePassword}>
              <input
                type="text"
                className="contact__inputField none"
                value={email}
                required
                readOnly
              ...
              <div className="m-10">
                <div className="contact__infoField">
                  ...
                  <input
                    ...
                    maxLength="60"
                    onChange={(event) =>
                      handlePassword("oldPassword", event.target.value)
                    }
                ...
                <div className="m-10">
                  <div className="contact__infoField">
                    ...
                    <input
                      title="New Password"
                      id="new-password"
                      type={properties.newPassword ? "text" : "password"}
                      className="contact__inputField"
                      minLength="6"
                      maxLength="60"
                      onChange={(event) =>
                        handlePassword("newPassword", event.target.value)
                      }
                    ...
                    <IconButton
                      className="view-eye"
                      onClick={() =>
                        handleChange("newPassword", !properties.newPassword)
                      }
                      name="newPassword"
                    >
                      <FontAwesomeIcon
                        icon={properties.newPassword ? faEyeSlash : faEye}
                      />
                    </IconButton>
                  </div>
                </div>
                <div className="m-10">
                  <div className="contact__infoField">
                    ...
                    <input
                      ...
                      maxLength="60"
                      onChange={(event) =>
                        handlePassword("confirmPassword", event.target.value)
                      }
      ...
      <div id="mfa-bg" className="modal hiddenModal">
        <div id="mfa-modal" className="modal__container hiddenModal">
          <IconButton
            onClick={() => closeModal("mfa-bg", "mfa-modal")}
            className="float-right"
          >
            <FontAwesomeIcon
              icon={faTimes}
            ...
            <ol className="ml-40 ul-mb10">
              <li>
                ...
                <blockquote className="mt-20">
                  ...
                  <p className="mt-10">
                    Verification Code will be sent to <b>{email}</b> via email
                    and will be valid for only 5 (five) minutes. Once you enable
                    2 Factor Authentication (2FA), you will be prompted to enter
                    verification code on every login session.
                  </p>
                </blockquote>
                <button
                  className="oauth-box google isCentered block mt-20 p-12 button"
              ...
              <li className="mt-20">
                ...
                <blockquote className="mt-20">
                  ...
                  <p className="mt-10">
                    Note: If you do not have access to both your account or
                    email, we are unable to remove 2FA and you will have to
                ...
                <form onSubmit={VerifyOTP}>
                  <div className="m-10">
                    <div className="contact__infoField">
                      ...
                      <div
                        id="otp1"
                        className="otp flex-wrap justify-center isCentered"
                      >
                        <input id="token-1" />
                        <input id="token-2" />
                        <input id="token-3" />
                        ...
                        <input id="token-6" />
                        {data.isBackupCode
                          ? [<input id="token-7" />, <input id="token-8" />]
                          : null}
                      </div>
                    </div>
                  </div>
                  {authenticated && security["2FA"] ? (
                    <p className="isCentered">
                      If you're unable to receive a security code, use one of
                      your{" "}
                      <button
                        type="button"
                        className="link-btn link"
                        onClick={() => {
                          handleData("isBackupCode", !data.isBackupCode);
                          document.getElementById("token-1").focus();
                        }}
                      >
                        Backup Codes
      ...
      <div id="otp-bg" className="modal hiddenModal">
        <div id="otp-modal" className="modal__container hiddenModal">
          <IconButton
            onClick={() => closeModal("otp-bg", "otp-modal")}
            className="float-right"
          >
            <FontAwesomeIcon
              icon={faTimes}
            ...
            <ol className="ml-40 ul-mb10">
              <li>
                ...
                <blockquote className="mt-20">
                  ...
                  <p className="mt-10">
                    In order to improve our services, qualities, and securities,
                    we will need an <b>OTP Token</b> before users perform Change
                    Password Request.
                  </p>
                </blockquote>
                <button
                  className="oauth-box google isCentered block mt-20 mb-10 p-12 button"
                  id="send-otp-pass"
                  onClick={sendOTP}
                >
                  Send Verification Code
                  ...
                  <div className="m-10">
                    <div className="contact__infoField">
                      ...
                      >
                        ...
                        {data.isBackupCode
                          ? [
                              <input id="otp-token-7" />,
                              <input id="otp-token-8" />,
                            ]
                          : null}
                      </div>
                    </div>
      ...
      {authenticated && security["2FA"] ? (
        <textarea
          id="backup-codes"
          className="contact__inputField no-bot"
          value={BackupCode()}
          readOnly
        />
      ) : null}
      {authenticated && security["2FA"] ? (
        <div id="backup-code-bg" className="modal hiddenModal">
          <div id="backup-code-modal" className="modal__container hiddenModal">
            <IconButton
              onClick={() => closeModal("backup-code-bg", "backup-code-modal")}
              className="float-right"
            >
              <FontAwesomeIcon
                icon={faTimes}
            ...
            <div className="modal__body">
              <p className="mb-10 monospace">
                Keep these backup codes somewhere safe but accessible. Each
                backup code can only be used once.
              </p>
              <div
                className="monospace"
                dangerouslySetInnerHTML={{ __html: sanitizer(BackupCodes()) }}
              ></div>
              <button
                className="oauth-box google isCentered block mt-20 mb-10 p-12 button"
                id="generate-token"
              ...
              <div className="contact__container isCentered no-padding">
                <p className="pr-10">
                  <button
                    className="oauth-box google isCentered block mt-10 mb-10 p-12 button"
                    id="copy-code"
                    onClick={CopyCode}
                  >
                    Copy to Clipboard
                ...
                <p className="pl-10">
                  <button
                    className="oauth-box google isCentered block mt-10 p-12 button"
                    id="download-code"
